...
export const SECRET_COLUMNS: Record<string, readonly string[]> = {
	...
	// A worktree's branch database answers on a connection string that carries
	// its own password — generated by the provider and never typed by anyone, so
	// this row is the only place it exists.
	worktree_branches: ['connection_json'],
	// What a git identity proves itself to a remote with. The public key is not
	// listed: it is public, and keeping it readable lets the UI show it for
	// copy-to-GitHub without opening the envelope.
	git_identities: ['ssh_private_key', 'ssh_passphrase', 'https_token']
} as const;
