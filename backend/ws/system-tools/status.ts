...
function toRecipeDTO(tool: 'git' | 'claude' | 'opencode' | 'chrome' | 'cloudflared', recipe: Awaited<ReturnType<typeof resolveRecipe>>) {
	return {
		tool,
...
export const systemToolsStatusHandler = createRouter()
	...
	.http('system-tools:status-all', {
		...
		const ids = ['git', 'claude', 'opencode', 'chrome', 'cloudflared'] as const;
