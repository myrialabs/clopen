<script lang="ts">
	...
	// --- Authoring (AI generation of Skills/Commands/Subagents/Instructions) ---

	const artifactGen = $derived(settings.artifactGenerator);
	const authoringCustom = $derived(!!artifactGen?.useCustomModel);
	const authoringEngine = $derived(authoringCustom && artifactGen ? artifactGen.engine : settings.selectedEngine);
	const authoringModelId = $derived(authoringCustom && artifactGen ? artifactGen.modelId : settings.selectedModelId);
	const authoringEngineMeta = $derived(ENGINES.find(e => e.type === authoringEngine));
	const authoringModelMeta = $derived(modelStore.getById(authoringModelId));
	...
	function handleAuthoringModelChange(modelId: string) {
		const base = baseArtifactGen();
		const model = modelStore.getById(modelId);
		updateSettings({
...
<div class="py-1">
	...
	{#if activeTab === 'authoring'}
		...
		<button type="button" class="flex items-center gap-3 w-full text-left mb-4" onclick={toggleAuthoringCustom}>
			<div class="relative w-9 h-5 rounded-full transition-colors duration-200 flex-shrink-0
				{authoringCustom ? 'bg-violet-600' : 'bg-slate-300 dark:bg-slate-600'}">
				<div class="absolute top-0.5 w-4 h-4 rounded-full bg-white shadow-sm transition-transform duration-200
					{authoringCustom ? 'translate-x-4.5' : 'translate-x-0.5'}"></div>
			</div>
			<div>
		...
		{#if authoringCustom}
			<EngineModelPicker
				...
				onEngineChange={handleAuthoringEngineChange}
				onModelChange={handleAuthoringModelChange}
			/>
		{:else}
			<div class="flex items-center gap-3 px-3.5 py-2.5 border border-slate-200 dark:border-slate-700 rounded-lg bg-slate-50 dark:bg-slate-800/50">
				{#if authoringEngineMeta}
					<div class="flex-shrink-0">
						<div class="flex dark:hidden items-center justify-center w-4 h-4">{@html authoringEngineMeta.icon.light}</div>
						<div class="hidden dark:flex items-center justify-center w-4 h-4">{@html authoringEngineMeta.icon.dark}</div>
					</div>
				{/if}
				<div class="flex-1 min-w-0">
					<span class="text-sm font-medium text-slate-900 dark:text-slate-100">{authoringModelMeta?.engine.model.name || authoringModelId}</span>
					<span class="text-xs text-slate-500 dark:text-slate-400 ml-1.5">(same as assistant)</span>
				</div>
			</div>
		{/if}
