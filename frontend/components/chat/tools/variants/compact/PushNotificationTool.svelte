...
<div class="space-y-0.5 text-sm">
	<div class="flex items-center flex-wrap gap-x-1.5 gap-y-0.5">
		...
		<span class="font-medium text-slate-800 dark:text-slate-200 truncate">{input.title || 'notification'}</span>
	</div>
	{#if input.message}
		<div class="text-xs text-slate-400 dark:text-slate-500 truncate">{input.message}</div>
	{/if}
</div>
