<script lang="ts">
	import type { TextareaProps } from '$shared/types/ui/components';
	...
	let {
		...
		class: className = '',
		...props
	}: TextareaProps & { class?: string } = $props();

	// Generate unique ID for textarea
	const textareaId = `textarea-${Math.random().toString(36).substr(2, 9)}`;

	// Generate modern AI textarea classes
	const baseClasses =
...
<div class="space-y-1">
	{#if label}
		<label for={textareaId} class="block text-sm font-semibold text-slate-700 dark:text-slate-300">
			{label}
			{#if required}
